        ...
        >
          <p className="text-right font-lpmq text-2xl text-arab dark:text-text-bg-dark">
            ...
            <span className="text-3xl">
              {arabicNumber(data.number.inSurah)}
            </span>
          </p>
          <p className="mt-6">{data.translation.id}</p>
          <div className="flex items-center justify-end mt-5 gap-5">
            <button
              onClick={() => handleCopy(data.text.arab, data.number.inSurah)}
              className="group"
            >
            ...
            <button
              onClick={() => handleShowModal(data.number.inSurah)}
              className="group"
            >
              <div className="flex flex-col items-center">
              ...
              <div className="flex flex-col items-center ">
                {currentVerse === data.number.inSurah && isPlaying ? (
                  <>
                    ...
                    <p className="text-sm group-hover:text-primary">Pause</p>
                  </>
                ) : (
                  <>
                    <CiPlay1 className="text-xl group-hover:text-primary" />
                    <p className="text-sm group-hover:text-primary">Play</p>
                  </>
                )}
              </div>
            </button>
